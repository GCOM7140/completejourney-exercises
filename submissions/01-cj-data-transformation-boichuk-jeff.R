...
# Mo's bar chart of product brand
product %>%
ggplot(aes(brand)) +
  geom_bar()

# Age and Marital Status, A = Married, B = Single, U = Unknown

library(completejourney)
library(ggplot2) 

ggplot(hh_demographic, aes(x = age_desc)) +
  geom_bar(aes(color = marital_status_code), size = 1.65) +
  coord_flip()
 # you could also fill the bars, but it is easier to see with just the black-ish color
