...
get_data(which = "demographics", verbose = FALSE)
...
#Question 2: How could you plot median spend per individual by household size to answer this new question with a visualization
transactions_prices %>%
  inner_join(demographics, by = "household_id") %>% 
  mutate(
    household_size = str_replace(household_size, "5\\+", "5") %>% 
