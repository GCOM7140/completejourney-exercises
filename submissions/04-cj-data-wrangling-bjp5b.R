...
#' 1,149 households didn't reedem a coupon
...
left_join(
  ...
  by = "coupon_upc") %>% 
  summarize(redemption_rate = sum(!is.na(n_redemptions)))
#' 491 out of 981 (about 50%) redeemed at least once
...
transactions %>% 
  ...
  head(5)
#'  Instant Formula (grew 53.4%), Seafood - Frozen (grew 29.8%), Candy -
#'  packaged (grew 23.3%), Oral Hygiene products (grew 19.5%), Domsestic Wine
#'  (grew 13.5%)
