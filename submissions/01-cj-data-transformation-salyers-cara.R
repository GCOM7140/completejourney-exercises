...
# Change the discount variables (i.e., retail_disc, coupon_disc, coupon_match_disc) from negative to
transaction_data <- transaction_data %>%
...
transaction_data %>%
  ...
  select(product_id) %>%
  n_distinct()
# ANSWER: Product w/ regular price less than $1 = 12442

transaction_data %>%
  filter(loyalty_price <= 1) %>%
  select(product_id) %>%
  n_distinct()
# ANSWER: Product w/ regular price less than $1 = 20113 

transaction_data %>%
  filter(coupon_price <= 1) %>%
  select(product_id) %>%
  n_distinct()
# ANSWER: Product w/ regular price less than $1 = 22273
...
transaction_data %>%
  ...
  ungroup() %>%
  summarise(prop_over10 = mean(basket_value > 10))
# ANSWER: 0.654
