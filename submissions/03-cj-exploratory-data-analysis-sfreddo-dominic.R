...
my_transaction_data <- left_join(transaction_data, product) %>% 
  ...
  mutate(regular_price  = (sales_value + retail_disc + coupon_match_disc) / quantity,
         ...
         purchase_price = ifelse(coupon_disc > 0, coupon_price, 
                                 ifelse(retail_disc > 0, loyalty_price, regular_price)))
?ifelse
...
#2500 in my_transaction_data and 801 have info in hh_demographic

#2

exercise_2 <- inner_join(my_transaction_data, hh_demographic) %>% 
  mutate(hh_size = str_replace(household_size_desc, '5\\+', '5') %>% 
  as.integer()) %>% 
  group_by(household_key, week_no) %>% 
  summarize(total_spend = sum(purchase_price, na.rm = TRUE),
            hh_size = max(hh_size, na.rm = TRUE)) %>% 
  ungroup() %>%
  mutate(wkly_spend_per_ind = total_spend / hh_size)

exercise_2 %>% 
  summarize(med_wkly_spend_per_ind = median(wkly_spend_per_ind, na.rm = TRUE))

#The median spend per individual is $19.4
...
#It would seem that there is a slightly higher chance of a basket containing beer if it also contains diapers.
...
inner_join(my_transaction_data, hh_demographic) %>% 
  ...
  ggplot() +
  geom_col(aes(x = income_desc, y = total_spend, fill = brand), position = 'fill') + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

#The same plot just done using code from a previous submission.
...
brand_data <- my_transaction_data %>% 
  ...
  mutate(income_desc = factor(income_desc, 
                              ...
                              ordered = TRUE)) %>% 
  group_by(income_desc, brand) %>% 
  summarise(total_spent = sum(purchase_price))

brand_data %>% 
  ggplot(aes(income_desc, total_spent)) +
