...
#Question 1
  ...
  nrow()
  #there are 2500 households overall
  
  inner_join(my_transaction_data, hh_demographic) %>% 
  distinct(household_key) %>% 
  nrow()
  #801 households when seeing only those that match both tibbles and not counting duplicates
...
#Question 2
  inner_join(my_transaction_data, hh_demographic) %>% 
    ...
    exercise_2 %>% 
    summarize(medianspendperweek = median(wkly_spend_per_ind, na.rm = TRUE))
