...
#--------------------------------------------------------
#Assignment 3: Complete Journey Exploratory Data Analysis
#--------------------------------------------------------
...
exercise_2 %>%
  ...
  ggplot(mapping = aes(x = hh_size, y = med_wkly_spend_per_ind)) +
...
#Given that the lift is greater than one, the probability of the purchase of diapers and beer are independent of each other. In other words, the association between the two products is not significant. 
...
inner_join(my_transaction_data, hh_demographic) %>% 
  ...
  ggplot(mapping = aes(x = income_desc, y = total_spend, fill = brand)) +
