...
transaction_data <- transaction_data %>% 
  ...
  summarize(number_coupon_redemp = coupon_redempt)

campaign_table <- campaign_table %>% 
  distinct(household_key)

transaction_data %>% 
  left_join(campaign_table, by = "household_key") %>% 
  percent_campaign
...
```{r}
hh_coupon_counts <- coupon_redempt %>% 
  count(household_key)
hh_campaign_participants <- campaign_table %>% 
  distinct(household_key)

hh_redemption_check <- anti_join(hh_campaign_participants, 
                                 hh_coupon_counts, by = 'household_key')

(nrow(hh_redemption_check))
```
...
```{r}
product_coupon <- inner_join(product, coupon, by = "product_id")

new <- inner_join(product_coupon, coupon_redempt, by = c("campaign", "coupon_upc"))
