...
# Looks terrible. The bar is very long and tall. Could be because the count on a quantity of 0 is too large to see the rest. maybe we filter quantity down.

ggplot(data = transactions %>% filter(quantity <= 10)) +
  geom_histogram(mapping = aes(x = quantity))

# much better graph allowing us to see better the quantities
...
# Very messy graph but we can see the peaks and valleys throughout the year.
...
# The total sales for national brands is more than double that of private brandsS
...
#National brand is preferred for soft drinks however the local brands are preferred for cheese.
