...
#Question 1: Explore the distribution of quantity. What, if anything, do you
#find unusual about this visualization?
  
ggplot(data = transactions) +
  geom_histogram(mapping = aes(x = quantity))

#It is unusual that there are an overwhelming number of instances where the quantity is equal to zero
...
#You can get dates from the transaction_timestamp variable in the transactions
#dataset with the date() function of the lubridate package. If you are
#interested, you can see some examples of other accessor functions in the
#lubridate package here.
...
transactions %>%
  ...
  ggplot() + 
    geom_smooth(mapping = aes(x = date, y = total_sales))
...
transactions_products %>%
  group_by(brand) %>%
  summarise(total_sales_vol = sum(sales_value)) %>%
  ggplot(mapping = aes(x = brand, y = total_sales_vol)) +
...
transactions_products %>%
  ...
  summarise(count = n()) %>%
  ggplot() +
    geom_bar(mapping = aes(x = package_size %>% fct_reorder(count),
                           y = count), stat = "identity") +
  coord_flip()
  
#The most popular product is the 18oz product  
