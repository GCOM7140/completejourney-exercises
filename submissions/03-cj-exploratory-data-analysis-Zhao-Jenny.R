...
inner_join(my_transaction_data, hh_demographic) %>% 
  distinct(household_key) %>% 
  nrow()
# There are 2500 unique households exist in my_transaction_data, and 801 of these households in my_transaction_data have demographic data in hh_demographic?
...
# The median weekly spend per individual is 19.4
...
# baskets with diapers in them are more likely than average to have beer in them.
...
# the association between these products practically significant in my_transaction_data since the lift is greater than 1.
