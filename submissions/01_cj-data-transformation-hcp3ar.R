...
transactions <- transactions %>% 
                  mutate(retail_disc_pos = abs(retail_disc),
...
transactions %>% 
  ...
  ungroup() %>%
  summarize(proportion_over_10 = round(mean(basket_value > 10), 2))
  summarise(proportion = n_distinct(sales_value))
...
transactions %>%
  ...
  mutate(pct_loyalty_discount = (1 - (loyalty_price / regular_price))) %>%
  group_by(store_id) %>%
  summarize(store_rev = sum(sales_value),
            store_discount = mean(pct_loyalty_discount)
            ) %>%
  filter(store_rev > 10000) %>%
