...
exercise_2 %>% 
  group_by(hh_size) %>% 
  summarize(med_wkly_spend_per_individual = median(wkly_spend_per_ind)) %>% 
  ggplot() +
  geom_bar(mapping = aes(x = hh_size, y = med_wkly_spend_per_individual), stat = 'identity')
...
inner_join(my_transaction_data, product) %>% 
  ...
  summarize(
    prop_both   = sum(basket_with_diapers * basket_with_beer == 1) / 
      sum(basket_with_diapers == 1),
    prob_beer   = mean(basket_with_beer),
    diaper_lift = prop_both / prob_beer
  ) # Lift for diaplers = 1.06
...
# More money spent on national brands over all. Lower income households more likely to choose private lable brands than higher income households.