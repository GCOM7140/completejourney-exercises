...
transactions %>%
  ...
  ungroup() %>%
  summarise(proportion = mean(basket_value > 10)*100)
...
transactions %>%
  ...
  group_by(store_id) %>%
  summarise (store_sales_value = sum(sales_value),
             avg_store_disc = mean(pct_loyalty_disc)
             ) %>%
  filter (store_sales_value > 10000) %>%
  arrange(desc(avg_store_disc))
