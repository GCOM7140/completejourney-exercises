...
transactions %>%
  ...
  ungroup() %>%
  summarize(prop10 = mean(basket_value > 10) * 100)

## Question 5 - Which store with over $10K in total sales_value discounts its products the most for loyal customers?

transactions %>%
  filter(!is.na(loyalty_price), !is.na(regular_price)) %>%
  mutate(pct_loyalty_disc = 1 - (loyalty_price/regular_price)) %>%
  group_by(store_id) %>%
  summarize(
    store_sales_value = sum(sales_value),
    store_pct_loyalty = mean(pct_loyalty_disc)
  ) %>%
  filter(store_sales_value > 10000) %>%
  arrange(desc(store_pct_loyalty))
