...
transactions_prices %>%
  ...
  mutate(
    ...
    spend_weekly_per_ind = spend_total / household_size
  ) %>%
  ungroup() %>%
  summarize(
    spend_weekly_per_ind_med = median(spend_weekly_per_ind, na.rm = TRUE)
  ) %>%
  ggplot(mapping = aes(x = household_size, y = spend_weekly_per_ind)) + 
  geom_point()
...
transactions_prices %>% 
  ...
  summarize(
    prob_both = sum(basket_has_diapers * basket_has_beer == 1, na.rm = TRUE)
                  / sum(basket_has_diapers == 1, na.rm = TRUE),
    prob_beer = mean(basket_has_beer == 1, na.rm = TRUE),
    diaper_lift = prob_both / prob_beer
  )

# according to this data set there is no diaper lift
